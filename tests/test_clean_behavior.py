...
class TestCleanWithPrefix:
    ...
    def test_clean_with_prefix_finds_pool_files(self, capfd):
        """Clean command with prefix correctly lists pool files under the prefix.

        This test verifies that when prefix is set (e.g., "apt"), the clean command
        ...
        # Upload an orphaned .deb file to the pool
        self._upload_deb_to_pool("tests/fixtures/hello_2.10-5_amd64.deb")

        # Verify the orphan exists in S3 - this should find files under "apt/pool/"
        # Note: We call clean_command with prefix="apt" to trigger the double-prefix bug
        # The bug causes s3_list_objects to look for "apt/apt/pool/" instead of "apt/pool/"
        pool_files_before = self._get_pool_files()
        assert any("hello" in f for f in pool_files_before), (
            f"Orphan file should exist before clean. Pool files found: {pool_files_before}"
        )

    def test_clean_with_prefix_removes_orphans(self, capfd):
        """Clean command removes orphaned files when prefix is configured.

        This test verifies the double-prefix bug: when prefix="apt" is passed to
        clean_command, it constructs prefix_path="apt/pool/" and passes it to
        ...
        # Run clean command WITH prefix - this triggers the double-prefix bug
        # The bug: clean_command builds "apt/pool/" then s3_list_objects adds "apt/" again
        clean_command(
            bucket="test-bucket",
            codename="stable",
            component="main",
            prefix="apt",  # This triggers the double-prefix bug
        )
        ...
        # The hello package should be removed as it's not in manifest
        # With the bug, this fails because s3_list_objects looks for "apt/apt/pool/" (not found)
        assert not any("hello" in f for f in pool_files_after), "Orphan file should be removed by clean"
        assert "Removing" in output or "Removed" in output, "Clean should report removal"
